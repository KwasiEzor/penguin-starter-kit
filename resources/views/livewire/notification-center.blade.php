<div x-data="{ open: false }" class="relative">
    ...
    <button x-on:click="open = !open"
        x-bind:class="open ? 'bg-primary/10 dark:bg-primary-dark/10 text-on-surface-strong dark:text-on-surface-dark-strong' : ''"
        ...
        @if ($unreadCount > 0)
            <span class="ml-auto flex size-5 shrink-0 items-center justify-center rounded-full bg-danger text-[10px] font-bold text-on-danger">
            ...
            @forelse ($notifications as $notification)
                <div wire:key="notification-{{ $notification->id }}"
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="truncate text-sm text-on-surface-strong dark:text-on-surface-dark-strong {{ $notification->unread() ? 'font-medium' : '' }}">
