    ...
    <div x-cloak x-show="showSearch" x-transition class="fixed inset-x-0 top-[20%] z-50 mx-auto w-full max-w-lg">
        ...
        >
            ...
            <div class="flex items-center gap-3 border-b border-outline px-4 dark:border-outline-dark">
                <svg
                    class="size-5 shrink-0 text-on-surface dark:text-on-surface-dark"
                ...
                />
                <kbd
                    class="hidden shrink-0 rounded border border-outline px-1.5 py-0.5 text-[10px] font-medium text-on-surface dark:border-outline-dark dark:text-on-surface-dark sm:inline"
                >
                    ESC
                </kbd>
            </div>
            ...
            <div class="max-h-80 overflow-y-auto">
                @if ($pages->count() && $query)
                    <div class="px-3 py-2">
                        <p
                            class="px-1 text-xs font-medium uppercase tracking-wider text-on-surface dark:text-on-surface-dark"
                        >
                            {{ __('Pages') }}
                        </p>
                    </div>
                ...
                @if (count($results))
                    <div
                        class="px-3 py-2 {{ $pages->count() ? 'border-t border-outline dark:border-outline-dark' : '' }}"
                    >
                        <p
                            class="px-1 text-xs font-medium uppercase tracking-wider text-on-surface dark:text-on-surface-dark"
                        >
                    ...
                    @foreach ($results as $post)
                        ...
                        >
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="truncate font-medium text-on-surface-strong dark:text-on-surface-dark-strong">
                                    {{ $post->title }}
                                </p>
                                <p class="text-xs text-on-surface dark:text-on-surface-dark">
                                    {{ ucfirst($post->status) }} &middot; {{ $post->created_at->format('M d, Y') }}
                                </p>
                            </div>
                        </a>
                    @endforeach
            ...
            <!-- Footer -->
            <div
                class="flex items-center justify-between border-t border-outline px-4 py-2 text-xs text-on-surface dark:border-outline-dark dark:text-on-surface-dark"
            >
                <span>{{ __('Search') }}</span>
