<div class="flex flex-col gap-6">
    ...
    <div>
        ...
        <x-typography.subheading size="lg">{{ __('Manage your subscription and view order history') }}</x-typography.subheading>
    </div>
    ...
    <x-card>
        ...
        @if ($subscription && $subscription->active())
            ...
            @if ($hasStripeId)
                <div class="mt-4">
                    <x-button variant="outline" wire:click="redirectToPortal">{{ __('Manage Subscription') }}</x-button>
                </div>
            @endif
        @else
            <p class="text-sm text-on-surface dark:text-on-surface-dark">{{ __('You don\'t have an active subscription.') }}</p>
            <div class="mt-4">
        ...
        @if ($orders->count())
            <div class="overflow-x-auto -mx-4">
                <table class="w-full text-sm text-left">
                    <thead class="text-xs font-medium uppercase tracking-wider text-on-surface dark:text-on-surface-dark">
                        <tr>
                    ...
                    <tbody class="divide-y divide-outline dark:divide-outline-dark">
                        @foreach ($orders as $order)
                            <tr>
                                <td class="px-4 py-2 font-medium text-on-surface-strong dark:text-on-surface-dark-strong">
                                ...
                                <td class="px-4 py-2 text-on-surface dark:text-on-surface-dark">{{ $order->created_at->format('M d, Y') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
